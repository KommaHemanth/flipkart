...
public class OrderController {
	...
	@GetMapping("/getAllOrderByCustomer/{customerSequenceNumber}/{expirationDuration}")
	public ResponseEntity<?> getAllOrderBycustomer(@PathVariable Long customerSequenceNumber,
			@PathVariable Integer expirationDuration) {
		try {
			return ResponseEntity
					.ok(orderServiceInterface.getAllorderByCustomer(customerSequenceNumber, expirationDuration));
		} catch (TokenExpiredException e) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body(new OrderResponse(e.getErrorCode(), e.getStrMsg()));
