...
public class JwtDaoImpl implements JwtDao {
	...
	public CustomerBasicDetailsEntity findByUserName(String userName)
		...
		if (!resultList.isEmpty()) {
			...
			cbde.setCustomerPassword(custObj[1] != "" ? PasswordDecoder.getDecryptedPassword(String.valueOf(custObj[1]))
					: custObj[1].toString());
			...
			rolesAndPermissions
					.setCustomer_sequence_number(custObj[2] != null ? Long.valueOf(custObj[2].toString()) : null);
	...
	public void updateTokenDetails(Integer expirationTime, Boolean isRememberMeSelected, String jwt,
		...
		tokenRepo.updateToken(isRememberMeSelected, new Date(System.currentTimeMillis() + 1000 * 60 * expirationTime),
	...
	public void createTokenDetails(Integer expirationTime, Boolean isRememberMeSelected, Long customerSeqNumber) {
		tokenRepo.createToken(customerSeqNumber, CustomerBasicDetailsConstants.tokenType,
				...
				new Date(System.currentTimeMillis() + 1000 * 60 * expirationTime), new Date(), null, false,
